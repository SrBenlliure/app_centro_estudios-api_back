...
export async function userRecovery(req: Express.Request, res: Express.Response) {
    ...
    const mail = {
        ...
        html: `<html><body><p>Haga click en el siguiente <a href='${process.env.DOMAIN}/api/v1/users/recovery/${user.username}' target='blank'>enlace</a> para cambiar su password.</p></body></html>`
    }
    ...
    else res.status(404).json({"error": "No se encontró ningún usuario con este e-mail"})
}
